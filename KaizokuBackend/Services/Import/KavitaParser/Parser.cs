...
public static partial class Parser
{
    ...
    /// <summary>
    /// Extracts the scanlator/group from filename if present
    /// </summary>
    /// <param name="filename">Filename to parse</param>
    /// <returns>Scanlator name or empty string if not found</returns>
    public static string ParseScanlator(string filename)
    {
        // Very simple implementation that looks for [GroupName] or (GroupName)
        var bracketMatch = Regex.Match(filename, @"\[([^\[\]]+)\]", RegexOptions.IgnoreCase);
        if (bracketMatch.Success)
        {
            return bracketMatch.Groups[1].Value.Trim();
        }

        var parenMatch = Regex.Match(filename, @"\(([^\(\)]+)\)", RegexOptions.IgnoreCase);
